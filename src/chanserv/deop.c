...
*****************************************************************************/
/* $Id: deop.c,v 1.3 2003/03/01 16:47:04 cure Exp $ */
...
#define CHANSERV_DEOP_OK              "%d users were deopped in %s."

/**************************************************************************************************
 * chanserv_deop
 **************************************************************************************************
 *   DEOP <#channel> [nick {[nick},...]
 *     Set mode -o on [nick] in <#channel>. If [nick] not specified, the user is deopped.
...
FUNC_COMMAND(chanserv_deop)
{
  ...
  if (count)
  {
    ...
    log_command(LOG_CHANSERV, from, "DEOP", "%s %s", chanbuf, queue_escape_string(bufn));
  }
  
  chanserv_dbase_update_lastlogin(ch);

  return com_message(sock, conf->cs->numeric, from->numeric, format, CHANSERV_DEOP_OK, count, chan);
}
