...
*****************************************************************************/
/* $Id: kickban.c,v 1.3 2003/03/01 16:47:04 cure Exp $ */
 ...
 *      kicks <nick> from <#channel>, and place a *!user@host ban, matching <nick>'s user@host
...
FUNC_COMMAND(chanserv_kickban)
{
  ...
  /* Is the user we are trying to kickban in out database, ie. online */
  ...
  if ((level = chanserv_dbase_check_access(from->nickserv, ch, command_info->level)))
  {
    ...
    /* Do the actually banning and kicking */
    com_send(irc, "%s M %s +b *!%s@%s\n", conf->cs->numeric, chan, info->username, info->host);
    com_send(irc, "%s K %s %s :%s\n", conf->cs->numeric, chan, info->numeric, reason);
    /* Keep the internal database synced */
    snprintf(buf, BUFFER_SIZE, "*!%s@%s", info->username, info->host);
    channels_addban(-1, chan, buf);
    channels_userpart(-1, chan, info->numeric);

    strcpy(buf, queue_escape_string(victim));
    strcpy(rbuf, queue_escape_string(reason));
    log_command(LOG_CHANSERV, from, "KICKBAN", "%s %s %s", queue_escape_string(chan), buf, rbuf); 
